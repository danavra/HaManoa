...
try:
    ...
    print("Module BeautifulSoup4 is missing!" + str(e))
    exit(1)
    ...
    if useColors == "true":
        if level <= int(loglevel):
            if level == 0:
                ...
                print(colored(datetime.now().strftime(
                    '%H:%M:%S') + " " + logString, "green"))
            elif level == 2:
                print(colored(datetime.now().strftime(
                    '%H:%M:%S') + " " + logString, "yellow"))
            elif level == 3:
                print(colored(datetime.now().strftime(
                    '%H:%M:%S') + " " + logString, "red"))
            elif level == 4:
                print(colored(datetime.now().strftime(
                    '%H:%M:%S') + " " + logString, "magenta"))
            elif level == 5:
                print(colored(datetime.now().strftime(
                    '%H:%M:%S') + " " + logString, "cyan"))
    else:
        if level <= int(loglevel):
            if level == 0:
                print(datetime.now().strftime('%H:%M:%S') + " " + logString)
...
def checkQuotationMarks(settingString):
    ...
    if settingString is not None and len(settingString) > 2 and settingString[0] == "\"" and settingString[-1] == "\"":
...
def addQuestionmarkIfNeeded(settingString):
    if settingString is not None and not settingString[-1] == "?":
        settingString = settingString + "?"
    return settingString
...
def checkBool(variable, name):
    if variable == "true" or variable == "false":
        ...
        log("Error parsing Variable. Please check the config file for variable: " +
            name + ". This variable should be 'true' or 'false'", 0)
        exit()
...
def checkInt(variable, name):
    if variable.isdigit():
        ...
        log("Error parsing Variable. Please check the config file for variable: " +
            name + ". This variable should be an integer", 0)
        exit()
...
def checkConf(cat, name):
    ...
    try:
        ...
        log("Variable in config is missing. Please set the variable: " +
            name + " in the config file in section: " + cat, 0)
        exit()
...
doCrawlFiles = checkConf("crawl", "okFileTypes")
listDoCrawl = doCrawlFiles.split(",")
...
def donwloadFile(downloadFileResponse):
    ...
    while True:
        ...
        if (endtime - starttime).total_seconds() >= 1:
            bytespersec = (bytes_so_far - bytesSineLasteCalc) / \
        ...
        if not header:
            ...
            if tenmegabyte >= 10485760:
                tenmegabyte = 0
                log("Downloaded %d bytes (%dByts/s | %0.2f%%MByts/s)" %
                    (bytes_so_far, bytespersec, MBbytespersec), 0)

            log("Downloaded %d bytes" % (bytes_so_far), 5)

        else:
            progress(bytes_so_far, total_size, "%d/%dB %0.2fMB/s\r" %
                     (bytes_so_far, total_size, MBbytespersec))
...
def saveFile(webFileFilename, pathToSave, webFileContent, webFileResponse, webFileHref):
    ...
    try:
        ...
        log('File was not created: "file://' +
            file_name + '"' + "Exception: " + str(e))
        return file_name

    fileWasDeleted = False

    fileWasDeleted = searchfordumpsSpecific(
...
def addFileToLog(pageLink, filePath):
    ...
    logFileWriter.write(datetime.now().strftime(
        '%d.%m.%Y %H:%M:%S') + " " + pageLink + " saved to '" + filePath + "'\n")
    logFileWriter.close()
    global logFile
...
def addHashToLog(pageDir, calcHash):
    logFileWriter = io.open(crawlHistoryFile, 'ab')
    logFileWriter.write(datetime.now().strftime(
        '%d.%m.%Y %H:%M:%S') + " " + pageDir + " calculated hash " + calcHash + "\n")
    logFileWriter.close()
    global logFile
    logFileReader = io.open(crawlHistoryFile, 'rb')
    logFile = logFileReader.read()
...
def decodeFilename(fileName):
    ...
    htmlDecode = htmlDecode.replace('/', '-').replace('\\', '-').replace(' ', '-').replace(
        '#', '-').replace('%', '-').replace('&', '-').replace('{', '-').replace('}', '-').replace('<', '-')
    htmlDecode = htmlDecode.replace('>', '-').replace('*', '-').replace('?', '-').replace('$', '-').replace(
        '!', '-').replace(u'‘', '-').replace('|', '-').replace('=', '-').replace(u'`', '-').replace('+', '-')
...
def findOwnCourses(myCoursesURL):
    ...
    try:
        if crawlallcourses == "true":
            responseCourses = urllib2.urlopen(
                myCoursesURL + "my/index.php?mynumber=-2", timeout=10)
        else:
    ...
    for course_string in courseNameList:
        ...
        if not onlyCrawlCoursesCheck(course_link):
            log("Course " + course_name +
                " will not be crawled because the course id is not given in option 'onlycrawlcourses'.", 3)
            continue
...
def searchfordumpsSpecific(filepath, fileName, filetype, pathtoSearch):
    ...
    log('Scanning directory "' + pathtoSearch + '" (file: "file://' + filepath +
        '", filename: "' + fileName + '", filetype: "' + filetype + '")....', 5)
...
def logDuplicates(dubPath, oriPath):
    ...
    if os.path.isfile(dubLogPath):
        ...
        if not dubPath in dubLog:
            log('I will store information about the duplicates in the "file://' +
                dubLogPath + '" file.', 4)
            dubLogWriter = io.open(dubLogPath, 'ab')
            dubLogWriter.write(datetime.now().strftime(
                '%d.%m.%Y %H:%M:%S') + " " + dubPath + " was found in this place " + oriPath + "\n")
            dubLogWriter.close()

        else:
            log('This duplicates were found before and loged in the "file://' +
                dubLogPath + '" file earlier. Inform the project maintainer pleace.', 5)
...
def logExternalLink(extlink, extname, extLinkDir):
    ...
    while True:
        if not os.path.isfile(new_name):
            file_name = new_name
            log('I will store the external link ' +
                extlink + ' in "file://' + file_name + '".', 0)
...
Name[en_US]=""" + extname)
            ...
            if extlink in externallinks:
                file_name = new_name
                log('This link was stored in the "file://' +
                    file_name + '" file earlier.', 5)
    ...
    if boolExternalLinkStored == True:
        logFileWriter = io.open(crawlHistoryFile, 'ab')
        logFileWriter.write(datetime.now().strftime(
            '%d.%m.%Y %H:%M:%S') + " External: " + extlink + " saved to '" + file_name + "'\n")
        logFileWriter.close()
        global logFile
        logFileReader = io.open(crawlHistoryFile, 'rb')
        logFile = logFileReader.read()
...
def okFileType(pagelink):
    extension = pagelink.split("?")[0].split(".")[-1]
    if dontcrawl == "":
        return False

    if not extension is None and extension in listDoCrawl:
        return True
    return False
...
def crawlMoodlePage(pagelink, pagename, parentDir, calledFrom, depth=0, forbidrecrusionfor=[]):
    ...
    log("Check page: '" + pagelink + "' named: '" + pagename + "' found on: '" +
        calledFrom + "' depth: " + str(depth) + " / " + str(maxdepth), 2)
    ...
    if pageFileName is None or pageFileName == "":
        pageFileName = os.path.basename(
            urllib2.urlparse.urlsplit(pagelink).path)
...
# /quiz/ = hausaufgaben wichtig ?                | saveInPagedir      ??maybe do not save
    ...
    if doSave and okFileType(pagelink):
        pageFilePath = saveFile(pageFileName, pageSaveDir,
    ...
    if not course_section is None:
        for one_section in course_section:
            ...
            if not section_links is None:
                ...
                sectionDir = normPath(addSlashIfNeeded(
                    parentDir) + pagename + "/" + sectionname)
                ...
                for link in section_links:
                    ...
                    crawlMoodlePage(hrefPageLink, nextName, sectionDir,
                                    pagelink, (depth + 1), forbidrecrusionforNew)
    ...
    if not page_links is None:
        for link in page_links:
            ...
            crawlMoodlePage(hrefPageLink, nextName, nextParentDir,
                            pagelink, (depth + 1), forbidrecrusionforNew)
...
if useauthstate == "true":
    ...
    # get mainURL from login response (this is not normal)
    calledFrom = responseLogin.geturl()
    pagelink = actionLink
...
crawlHistoryFile = normPath(addSlashIfNeeded(
    root_directory) + ".crawlhistory.log")
...
for course in courses:
    log("Check course: '" + course[0] + "' ID: " +
        parse_qs(urlparse(course[1]).query)['id'][0])
