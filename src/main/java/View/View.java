...
//    public TextField fld_fileQueryOutput;
    ...
    private String convertedCorpusPath;
...
//        changeScript();
//        System.out.println("------------------SCRIPT HAS CHANGED----------------------");
//        runScript();
//        System.out.println("------------------SCRIPT DONE----------------------");
        String convCorpus = convertDox();
        System.out.println("------------------FILES CONVERTED----------------------");
        convCorpus = convCorpus.substring(0, convCorpus.lastIndexOf(System.getProperty("file.separator"))+1) + "c2txt";
        int numOfDocsProcessed = createDataBase(convCorpus);
...
//        String sysSeparator = System.getProperty("file.separator");
        String configPath = System.getProperty("user.dir") + sysSeparator + "Moodle-Downloader2" + sysSeparator + "Moodle-Downloader2"
                                                    + sysSeparator + "src" + sysSeparator + "config.ini";
        File config = new File(configPath);
        try {
            FileReader fileReader = new FileReader(config);
            FileWriter fileWriter = new FileWriter(new File(configPath.replace("config.ini", "config2.ini")));
            ...
            while (reader.ready()){
                ...
                if (line.startsWith("root_dir")){
                    ...
                    line += ("\"" + fld_outputPath.getText() + sysSeparator + "MoodleFiles" + "\"");
                    File outputDir = new File(fld_outputPath.getText() + sysSeparator + "MoodleFiles");
                    outputDir.mkdirs();
                }
                else if (line.startsWith("username")){
                    line = line.substring(0, line.indexOf("\""));
                    line += ("\"" + fld_username.getText() + "\"");
                }
                else if (line.startsWith("password")){
                    line = line.substring(0, line.indexOf("\""));
                    line += ("\"" + fld_password.getText() + "\"");
                }
                line += lineSeparator;
                writer.write(line);
            }
            ...
            fileReader.close();
            config.delete();
            File newConfig = new File(configPath.replace("config.ini", "config2.ini"));
            newConfig.renameTo(new File(configPath));
            File toDelete = new File(configPath.replace("config.ini", "config2.ini"));
            toDelete.delete();
        }
        catch (Exception e){
            System.out.println("changeScript");
        }
    }

    private void runScript(){
//        String sysSeparator = System.getProperty("file.separator");
        String scriptPath = System.getProperty("user.dir") + sysSeparator + "Moodle-Downloader2" + sysSeparator + "Moodle-Downloader2" +
                                                        sysSeparator + "src" + sysSeparator + "moodleCrawler.py";;
        String command = SystemUtils.IS_OS_WINDOWS?("C:"+ sysSeparator +"python27"+ sysSeparator + "python.exe " + scriptPath):"python";
        try {
            Process p = Runtime.getRuntime().exec(command);
            p.waitFor();
        }
        catch (Exception e){
            System.out.println("Bad Python!!!");
        }
    }

    private String convertDox(){
//        String sysSeparator = System.getProperty("file.separator");
        TextExtractor extractor = new TextExtractor(fld_outputPath.getText());
        convertedCorpusPath = System.getProperty("user.dir") + sysSeparator + "corpus" + sysSeparator + "c2txt";
        String ans = extractor.readDirectories();
        return ans;
    }
     ...
     */
    private int createDataBase(String convertedCorpus) {
        ReadFile readF = new ReadFile(convertedCorpus, fld_outputPath.getText(), true);
    ...
    private void showIndexSummary(int numOfDocsProcessed, long time) {
        ...
        summary.setHeaderText("Done");
//        summary.setTitle("SUMMARY");
//        summary.setHeaderText("INDEX CREATION COMPLETE");
        summary.setContentText("Done");
        summary.show();
...
//            stem = "not stemmed";
        try {
            File f = new File(fld_outputPath.getText() + sysSeparator + "dataBase" + sysSeparator + stem + sysSeparator + "dicToShow.txt");
            if (!f.exists())
...
//            stemm = "not stemmed";
        ...
        try {
            ...
            br.close();
            dicLoaededInfo = fld_outputPath.getText()+true;
            if(showInfo) {
...
//            fld_corpusPath.setText(properties.getProperty("corpus"));
            ...
            fld_username.setText(properties.getProperty("username"));
            fld_password.setText(properties.getProperty("password"));
...
//            pw.println("corpus=" + fld_corpusPath.getText().replace("\\", "\\\\"));
            ...
            pw.println("username=" + fld_username.getText());
            pw.println("password=" + fld_password.getText());
//            pw.println("stemmer=" + true);
...
//        gp_btns.getColumnConstraints().add(new ColumnConstraints(100));
        ...
        dialog.setTitle("Filter by Courses");
        dialog.showAndWait();
    }
    ...
    private Collection<String> getCitiesFromIndex() {
        ...
        sCityIdx += "\\dataBase\\stemmed\\cityIndex.txt".replace("\\", sysSeparator);
...
//                        showDocument(result.getDocumentID(lbl.getText()));
        ...
        Scene dialogScene = new Scene(dialogVbox, 1200, 500);
        dialog.setScene(dialogScene);
...
//        if(dictianary == null || (dicLoaededInfo != null && !dicLoaededInfo.equals(fld_outputPath.getText()))){
...
//            alert.show();
//            searcher = new Searcher(fld_searchQuary.getText(), fld_corpusPath.getText(), true, fld_outputPath.getText(), cb_semantics.isSelected(), selectedCitiesFilter,dictianary);
//            long start = System.nanoTime();
...
//    }
    ...
    public void searchPressed() {
        convertedCorpusPath = fld_outputPath.getText() + System.getProperty("file.separator") + "c2txt";
        if(dictianary == null || (dicLoaededInfo != null && !dicLoaededInfo.equals(fld_outputPath.getText()+true))){
            loadDictionary(false);
            if (dictianary==null)
                return;
        }
        try {
            if(!(new File(convertedCorpusPath).exists()))
                throw new Exception("path does not exist\n"+convertedCorpusPath);

            searcher = new Searcher(fld_searchQuary.getText(), convertedCorpusPath, true, fld_outputPath.getText(), false, selectedCitiesFilter,dictianary);
            if (false) {
